...
function NumField({
  label, value, onChange, prefix, suffix, note, step, min, max
}: {
  label: string
  value: number
...
function MethodCard({
  method, result, isRecommended, isPro, isLocked
}: {
  method: string
  ...
  if (isLocked) return (
    <div className="bg-ink border border-gold/10 rounded-xl p-5 relative overflow-hidden">
      <div className="absolute inset-0 backdrop-blur-sm bg-black/60 flex flex-col items-center justify-center z-10 rounded-xl">
        ...
        <Link href="/pricing" className="text-[12px] text-white/60 hover:text-white transition-colors underline underline-offset-2">
          Upgrade to unlock
        </Link>
      </div>
      <div className="font-mono text-[9px] tracking-widest uppercase text-white/30 mb-2">{method}</div>
  ...
  return (
    <div className={`bg-ink rounded-xl p-5 border transition-all ${
      isRecommended
        ? 'border-gold/40 bg-gold/[0.03]'
        : 'border-white/[0.07]'
    }`}>
      {isRecommended && (
        <div className="font-mono text-[8px] tracking-widest uppercase text-gold bg-gold/10 border border-gold/20 px-2 py-0.5 rounded-full inline-block mb-3">
          ★ Highest Payment
        </div>
      )}
      <div className="font-mono text-[9px] tracking-widest uppercase text-white/30 mb-1">{method}</div>
      <div className="font-syne font-bold text-[32px] text-white mb-0.5">
        {fmt(result.annualPayment)}
      </div>
      <div className="font-mono text-[10px] text-white/40 mb-4">
        per year · {fmt(result.monthlyPayment)}/mo
      </div>
...
function LockInTimeline({
  ...
  const fiveYearMark = Math.min(5, totalYears)
  const fiveYearPct = (fiveYearMark / totalYears) * 100
  ...
  return (
    <div className="bg-ink border border-white/[0.07] rounded-xl p-5 mb-6">
      <div className="font-mono text-[9px] tracking-widest uppercase text-white/25 mb-4">
        SEPP Lock-In Timeline
      </div>
      <div className="relative h-12 bg-white/[0.04] rounded-lg overflow-hidden mb-3">
        {/* Lock-in period */}
        <div
          className="absolute h-full bg-gold/20 border-r border-gold/40"
          style={{ width: '100%' }}
        />
        {/* 5-year mark */}
        {currentAge < 54.5 && (
          <div
            className="absolute h-full border-r-2 border-dashed border-white/30"
            style={{ left: `${fiveYearPct}%` }}
          />
        )}
        {/* 59½ mark */}
        {currentAge < 59.5 && (
          <div
            className="absolute h-full border-r-2 border-dashed border-green-400/50"
            style={{ left: `${Math.min(age595Pct, 99)}%` }}
          />
        )}
        {/* Labels */}
        <div className="absolute inset-0 flex items-center px-3">
          <span className="font-mono text-[9px] text-gold/70">
            Age {currentAge} → {lockInEndAge} · SEPP active
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 bg-red-950/30 border border-red-500/20 rounded-lg px-4 py-3">
        ...
        <div className="text-white/50 text-[12px] leading-relaxed">
          If you modify or stop the SEPP before age {lockInEndAge} ({lockInEndYear}), the IRS applies a 10% penalty
          retroactively to all prior distributions, plus interest. The only allowed change: a one-time
          irrevocable switch from amortization/annuitization to the RMD method.
        </div>
      </div>
    </div>
  )
...
export default function SEPP72tToolkit() {
  ...
  const set = (key: keyof SEPPInputs) => (value: any) => {
    ...
    <div className="max-w-5xl mx-auto px-5 py-10">
      <div className="mb-8">
        <div className="font-mono text-[9px] tracking-widest uppercase text-gold mb-2">
          IRS Notice 2022-6 · Three Methods
        </div>
        <h1 className="font-syne font-bold text-[clamp(24px,4vw,40px)] tracking-tight text-white mb-3">
          72t / SEPP Calculator
        </h1>
        <p className="text-white/50 text-[14px] leading-relaxed max-w-2xl">
          Calculate penalty-free distributions from your IRA or 401k before age 59½.
          Compare all three IRS-approved methods and find the highest allowed payment for your situation.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
        ...
        <div className="bg-ink border border-white/[0.07] rounded-xl p-5">
          ...
          <NumField
            ...
            prefix="$"
            step={10000}
            note="IRA or 401k balance at valuation date"
          />
          <NumField
            label="Current Age"
          ...
          <div className="mb-4">
            ...
            <select
              value={inputs.lifeTable}
              onChange={e => set('lifeTable')(e.target.value as LifeTable)}
            ...
            <div className="font-mono text-[8px] text-white/20 mt-0.5">
              Single life typically gives highest factor
            </div>
          </div>
          ...
          <div className="border-t border-white/[0.06] pt-4 mb-4">
            <div className="font-mono text-[9px] tracking-widest uppercase text-white/25 mb-3">
              Interest Rate (Notice 2022-6)
            </div>
            <NumField
              label="Federal Mid-Term AFR"
              value={(inputs.midTermAFR ?? 0.045) * 100}
              onChange={v => set('midTermAFR')(v / 100)}
              suffix="%"
              step={0.1}
              note="Check IRS.gov monthly — changes each month"
            />
            <div className="bg-gold/5 border border-gold/15 rounded-lg px-3 py-2 mb-3">
              ...
              <div className="font-mono text-[8px] text-white/20">
                Greater of 5% or 120% × AFR ({fmtPct((inputs.midTermAFR ?? 0.045) * 1.20)})
              </div>
            </div>
            <NumField
              label="Rate to Use"
              value={inputs.interestRate * 100}
              onChange={v => set('interestRate')(Math.min(v / 100, maxRate))}
              suffix="%"
              step={0.1}
              note={`Max: ${fmtPct(maxRate)} — higher rate = higher payment`}
            />
          </div>
          ...
          <div className="bg-navy/50 rounded-lg px-3 py-2 text-center">
            ...
            <div className="font-mono text-[9px] text-white/35 leading-relaxed">
              Not financial or tax advice. Consult a qualified tax advisor before starting a 72t plan.
            </div>
          </div>
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <MethodCard
                  method="RMD Method"
                  result={results.rmd}
                  isRecommended={results.recommendedMethod === 'rmd'}
                  isPro={isPro}
                  isLocked={false}
                />
                <MethodCard
                  method="Fixed Amortization"
                  result={results.amortization}
                  isRecommended={results.recommendedMethod === 'amortization'}
                  isPro={isPro}
                  isLocked={false}
                />
                <MethodCard
                  method="Fixed Annuitization"
                  result={results.annuitization}
                  isRecommended={results.recommendedMethod === 'annuitization'}
                  isPro={isPro}
                  isLocked={!isPro}
                />
              </div>
              ...
              <LockInTimeline
                ...
                lockInEndAge={results.amortization.lockInEndAge}
                lockInEndYear={results.amortization.lockInEndYear}
              />

              {/* Account splitting — Pro */}
              {isPro ? (
                <div className="bg-ink border border-white/[0.07] rounded-xl p-5 mb-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                      ...
                      className="font-mono text-[10px] tracking-widest uppercase text-gold/60 hover:text-gold border border-gold/20 px-3 py-1.5 rounded transition-colors"
                    >
                      {showSplit ? 'Hide' : 'Show'}
                    </button>
                  </div>
                  {showSplit && splitResult && (
                    <>
                      <p className="text-white/45 text-[13px] leading-relaxed mb-4">
                        Instead of running a SEPP on your entire account, split it first. Run the SEPP on just
                        the portion needed to hit your target income — keep the rest penalty-free at 59½.
                      </p>
                      <NumField
                      ...
                      <div className="grid grid-cols-2 gap-4 mt-4">
                        <div className="bg-gold/5 border border-gold/15 rounded-xl p-4">
                          ...
                          <div className="font-mono text-[10px] text-white/40 mt-1">
                            Generates {fmt(splitResult.primaryAnnualPayment)}/yr
                          </div>
                        </div>
                        <div className="bg-navy/50 border border-white/[0.06] rounded-xl p-4">
                          ...
                          <div className="font-mono text-[10px] text-white/40 mt-1">
                            Accessible at 59½ or second SEPP
                          </div>
                        </div>
                      </div>
                      <div className="mt-3 font-mono text-[11px] text-white/40 leading-relaxed bg-navy/30 rounded-lg p-3">
                        {splitResult.rationale}
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="bg-ink border border-gold/10 rounded-xl p-5 mb-6 flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white/40 text-[12px] leading-relaxed mb-3">
                      Run a SEPP on just part of your IRA — generate exactly the income you need while
                      keeping the rest growing penalty-free until 59½ or a future SEPP.
                    </p>
                    <Link
                      href="/pricing"
              ...
              <div className="bg-navy/30 border border-white/[0.06] rounded-xl p-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {[
                    { rule: 'Lock-in period', detail: `Longer of 5 years or until 59½. Starting at 50 = ${(Math.max(inputs.currentAge + 5, 59.5) - inputs.currentAge).toFixed(1)} years locked.` },
