...
export default function TaxableBrokerageAnalyzer() {
  ...
  const bridgeData = useMemo(() => {
    ...
    for (let age = retireAge; age <= 90; age++) {
      ...
      if (isBridge) { t = Math.max(0, t - need); k *= (1 + rate); r *= (1 + rate) }
      else { k = Math.max(0, k - need); t *= (1 + rate); r *= (1 + rate) }
      rows.push({ age, 'Taxable': Math.round(Math.max(0, t)), '401k': Math.round(Math.max(0, k)), 'Roth': Math.round(Math.max(0, r)) })
    }
    return rows
  }, [retireAge, taxable, k401k, roth, annualSpend, rate])

  const bridgeFullyCovered = !bridgeData.find(d => d['Taxable'] === 0 && d.age < Math.ceil(bridgeEnd))
  const taxSaved = useMemo(() => Math.round(annualSpend * bridgeYears * (0.30 - 0.05)), [annualSpend, bridgeYears])
  ...
  return (
    <div style={{ background: '#0D1420', borderRadius: 16, border: '1px solid rgba(232,184,75,0.15)', overflow: 'hidden', fontFamily: "'IBM Plex Mono', monospace", margin: '2rem 0' }}>
      <div style={{ background: '#141C28', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '20px 24px' }}>
        ...
        <h3 style={{ color: COLORS.white, fontSize: 18, fontFamily: 'Georgia, serif', fontWeight: 700, margin: 0, marginBottom: 4 }}>Taxable Brokerage Bridge Analyzer</h3>
        <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11, margin: 0 }}>See if your taxable account can fund your bridge — and how the three buckets work together from retirement to 90.</p>
      </div>
      ...
      <div style={{ padding: '24px' }}>
        ...
        <div style={{ background: '#141C28', borderRadius: 10, padding: '14px 16px', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 16 }}>
            {[{ label: 'Taxable', color: COLORS.teal, value: taxable }, { label: '401k / IRA', color: COLORS.gold, value: k401k }, { label: 'Roth', color: COLORS.purple, value: roth }].map(({ label, color, value }) => (
        ...
        {/* Bridge status */}
        <div style={{ background: bridgeFullyCovered ? 'rgba(74,222,128,0.06)' : 'rgba(248,113,113,0.06)', border: `1px solid ${bridgeFullyCovered ? COLORS.sage : COLORS.red}30`, borderLeft: `3px solid ${bridgeFullyCovered ? COLORS.sage : COLORS.red}`, borderRadius: 8, padding: '12px 14px', marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
          {[
            { label: 'Bridge Length', value: `${bridgeYears.toFixed(1)} yrs`, sub: `Age ${retireAge} → 59½`, color: COLORS.teal },
            { label: 'Tax Saved vs 401k First', value: formatDollars(taxSaved), sub: 'over bridge years', color: COLORS.sage },
            { label: '401k at 59½', value: formatDollars(Math.round(k401k * Math.pow(1 + rate, bridgeYears))), sub: `untouched at ${returnRate}%`, color: COLORS.gold },
        ...
        <div style={{ background: '#141C28', borderRadius: 12, padding: '16px', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 16 }}>
          ...
          {assetLocationData.map(({ account, best, avoid, color, reason }) => (
            <div key={account} style={{ display: 'flex', gap: 12, marginBottom: 12, alignItems: 'flex-start' }}>
              <div style={{ minWidth: 70, padding: '4px 8px', borderRadius: 4, background: `${color}20`, border: `1px solid ${color}40`, fontSize: 9, color, fontWeight: 600, textAlign: 'center' as const }}>{account}</div>
              <div>
        ...
        {!isPro && (
          <div style={{ background: 'linear-gradient(135deg, rgba(232,184,75,0.06) 0%, rgba(232,184,75,0.02) 100%)', border: '1px solid rgba(232,184,75,0.2)', borderLeft: '3px solid #E8B84B', borderRadius: 12, padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 20 }}>
            <div>
              ...
              <div style={{ fontSize: 14, fontFamily: 'Georgia, serif', fontWeight: 700, color: '#fff', marginBottom: 6 }}>Get your Bridge Risk Score in 60 seconds.</div>
