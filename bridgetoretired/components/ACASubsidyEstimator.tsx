...
const FPL_2026: Record<number, number> = { 1: 15_650, 2: 21_150, 3: 26_650, 4: 32_150 }
...
function calcSubsidy(magi: number, householdSize: number, age: number) {
  ...
  const maxContribution = Math.round(magi * contributionPct / 12)
  ...
  return { fpl, fplPercent, benchmarkPremium, maxContribution, monthlySubsidy, annualSubsidy, yourMonthlyCost, yourAnnualCost, eligible, overCliff }
}
...
const CustomTooltip = ({ active, payload, label }: any) => {
  ...
  return (
    <div style={{ background: '#0D1420', border: '1px solid rgba(232,184,75,0.2)', borderRadius: 8, padding: '10px 14px', fontFamily: 'monospace', fontSize: 11 }}>
      ...
      {payload.map((p: any) => (
        <div key={p.name} style={{ color: p.fill || p.color, marginBottom: 3 }}>{p.name}: {formatDollars(p.value)}/mo</div>
      ))}
    </div>
  )
}
...
const PRO_FEATURES = [
  ...
  { icon: '📄', label: 'PDF Report Export', sub: 'ACA costs + 30-yr plan, CPA-ready' },
]
...
export default function ACASubsidyEstimator() {
  ...
  const chartData = useMemo(() => {
    const fpl = FPL_2026[Math.min(householdSize, 4)] ?? 32_150
    return [100, 133, 150, 200, 250, 300, 350, 400, 450].map(pct => {
  ...
  const statusLabel = result.overCliff ? '⚠ Over 400% FPL — No Subsidy' : fplPercent < 100 ? '⚠ Under 100% FPL — Medicaid Zone' : `✓ Eligible — ${Math.round(fplPercent)}% of FPL`
  ...
  return (
    <div style={{ background: '#0D1420', borderRadius: 16, border: '1px solid rgba(232,184,75,0.15)', overflow: 'hidden', fontFamily: "'IBM Plex Mono', monospace", margin: '2rem 0' }}>
      <div style={{ background: '#141C28', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '20px 24px' }}>
        <div style={{ fontSize: 9, letterSpacing: 3, textTransform: 'uppercase', color: COLORS.gold, marginBottom: 6 }}>2026 Estimator</div>
        <h3 style={{ color: COLORS.white, fontSize: 18, fontFamily: 'Georgia, serif', fontWeight: 700, margin: 0, marginBottom: 4 }}>ACA Health Insurance Cost Estimator</h3>
        <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 11, margin: 0 }}>See your estimated monthly premium after subsidies — based on 2026 pre-ARPA rules. Subsidies end at 400% FPL.</p>
      </div>
      ...
      <div style={{ padding: '24px' }}>
        ...
        <div style={{ background: '#141C28', borderRadius: 10, padding: '14px 16px', border: `1px solid ${statusColor}40`, borderLeft: `3px solid ${statusColor}`, marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>FPL for {householdSize}-person household: ${result.fpl.toLocaleString()}</div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
          {[
            ...
            { label: 'Monthly Subsidy', value: formatDollars(result.monthlySubsidy), sub: `${formatDollars(result.annualSubsidy)}/year saved`, color: COLORS.teal },
            { label: 'Annual Cost', value: formatDollars(result.yourAnnualCost), sub: 'premiums only, not OOP', color: COLORS.red },
        ...
        <div style={{ background: '#141C28', borderRadius: 12, padding: '20px 16px 12px', border: '1px solid rgba(255,255,255,0.06)', marginBottom: 16 }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={chartData} margin={{ top: 5, right: 10, left: 10, bottom: 30 }}>
              ...
              <Bar dataKey="Your Cost" stackId="a">
                {chartData.map((entry, i) => <Cell key={i} fill={entry.isCurrentIncome ? COLORS.gold : COLORS.sage} opacity={0.85} />)}
              </Bar>
        ...
        <div style={{ background: 'rgba(232,184,75,0.06)', border: '1px solid rgba(232,184,75,0.15)', borderLeft: `3px solid ${COLORS.gold}`, borderRadius: 8, padding: '14px 16px', marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', margin: 0, lineHeight: 1.7 }}>
            {result.overCliff
              ...
              : `At $${magi.toLocaleString()} MAGI, you save ${formatDollars(result.annualSubsidy)}/year in subsidies. Each additional dollar above the 400% FPL cliff ($${(result.fpl * 4).toLocaleString()}) eliminates all subsidies. Plan Roth conversions carefully to stay under.`}
          </p>
        </div>
        ...
        <div style={{ background: 'rgba(45,212,191,0.06)', border: '1px solid rgba(45,212,191,0.15)', borderLeft: `3px solid ${COLORS.teal}`, borderRadius: 8, padding: '14px 16px', marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', margin: '0 0 10px', lineHeight: 1.7 }}>
            ACA costs are a major bridge year expense. Build them into your full retirement plan.
          </p>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' as const }}>
            ...
            <a href="/tools/roth-conversion-ladder-calculator" style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', border: '1px solid rgba(255,255,255,0.1)', padding: '5px 12px', borderRadius: 5, textDecoration: 'none' }}>Manage MAGI with Roth Ladder →</a>
