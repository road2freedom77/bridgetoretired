...
function runProjection(inputs: Inputs, ssAge: number, ssIncome: number): { data: YearData[], depleted: number | null } {
  ...
  for (let i = 0; i < totalYears; i++) {
    ...
    if (inputs.spendingMode === 'dynamic') {
      ...
      if (total < inputs.portfolio * 0.8) spending *= (1 - inputs.dynamicCutPct / 100)
    }
    ...
    if (inputs.withdrawalOrder === 'taxable-first' || isBridge) {
      ...
      if (remaining > 0 && !isBridge) {
        ...
        roth -= fromRoth
        trad = Math.max(0, trad - (remaining - fromRoth))
      } else if (remaining > 0 && isBridge) {
        roth -= Math.min(remaining, roth * 0.3)
      }
    ...
    trad    = Math.max(0, trad    * (1 + inputs.returnRate / 100))

    const total = taxable + roth + trad
    if (total <= 0 && depleted === null) depleted = age

    data.push({
      year: i + 1, age: Math.floor(age),
      spending: Math.round(totalSpending), income: Math.round(income),
...
const DEFAULT_INPUTS: Inputs = {
  ...
  portfolio: 1_150_000, taxable: 350_000, rothIRA: 150_000, trad401k: 650_000, cash: 75_000,
  annualSpending: 55_000, inflationRate: 2.5, returnRate: 6.5, equityPct: 65,
  ssAge62: 18_000, ssAge67: 24_000, ssAge70: 30_000,
  partTimeIncome: 0, partTimeYears: 0, healthcareCost: 8_400,
  withdrawalOrder: 'taxable-first', spendingMode: 'fixed', dynamicCutPct: 10,
}
...
export default function AdvancedCalculatorPage() {
  ...
  useEffect(() => {
    ...
    setInputs(prev => ({
      ...
      ssAge67:       active.ss_benefit   ?? prev.ssAge67,
      ssAge62:       Math.round((active.ss_benefit ?? prev.ssAge67) * 0.75),
      ssAge70:       Math.round((active.ss_benefit ?? prev.ssAge67) * 1.24),
  ...
  const spendingData = primaryScenario.data.map(d => ({
    age: d.age, Spending: d.spending, Income: d.income, Withdrawal: d.withdrawal,
  }))
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[380px_1fr] gap-8">
          <div className="space-y-5">
            ...
            {[
              ...
              { title: 'Portfolio Balances', fields: [
              ...
              { title: 'Returns & Inflation', fields: [
                  ...
                  {section.fields.map(f => (
                    <div key={f.key} className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <input type="number" min={(f as any).min} max={(f as any).max} step={(f as any).step}
                          value={(inputs as any)[f.key]} onChange={set(f.key as keyof Inputs)}
              ...
              <div className="p-4 space-y-2">
                {[
                  ...
                  <label key={opt.value} className="flex items-center gap-3 cursor-pointer">
                    <input type="radio" name="withdrawalOrder" value={opt.value}
              ...
              <div className="p-4 space-y-3">
                <div className="flex gap-3">
                  {[
                    ...
                    <label key={opt.value} className="flex items-center gap-2 cursor-pointer">
                      <input type="radio" name="spendingMode" value={opt.value}
                ...
                {inputs.spendingMode === 'dynamic' && (
                  <div className="flex items-center justify-between">
                    ...
                    <input type="number" min={5} max={30} step={1} value={inputs.dynamicCutPct}
                ...
                {[
                  ...
                  <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
                    className={`flex-1 py-3 font-mono text-[10px] tracking-widest uppercase transition-colors ${
                      activeTab === tab.key ? 'bg-[#1E2A3A] text-[#E8B84B] border-b-2 border-[#E8B84B]' : 'text-white/30 hover:text-white/60'
                    }`}
                  >
                    {tab.label}
                  </button>
