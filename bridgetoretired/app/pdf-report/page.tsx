...
import { useActiveScenario } from '@/lib/useActiveScenario'
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .header h1 { color: white; font-size: 26px; font-weight: 800; margin-bottom: 4px; }
  ...
  thead th { padding: 10px; font-size: 11px; font-weight: 700; color: white; text-align: left; text-transform: uppercase; }
...
<div class="page">
      ...
      ${[
        {
          ...
          desc: r.withdrawalRate > 4 ? `${r.withdrawalRate.toFixed(1)}% exceeds the 4% guideline.` : `${r.withdrawalRate.toFixed(1)}% is within a sustainable range.`
        },
        {
          ...
          desc: inputs.taxable >= (inputs.annualSpending + inputs.healthcareCost) * r.bridgeYears ? 'Taxable account fully funds the bridge period.' : `Taxable account covers ${Math.round(inputs.taxable / ((inputs.annualSpending + inputs.healthcareCost) * r.bridgeYears) * 100)}% of bridge needs.`
        },
        {
          label: 'Portfolio Longevity',
          status: r.funded ? 'ok' : 'bad',
          badge: r.funded ? `Funded to ${inputs.lifeExpectancy}` : `Depletes at ${r.depleted}`,
          desc: r.funded ? `Portfolio projects to ${fmt(r.at90)} at age 90.` : `Portfolio depletes at age ${r.depleted}.`
        },
        {
          label: 'SS Timing',
          status: inputs.ssAge >= 67 ? 'ok' : 'warn',
          badge: inputs.ssAge >= 67 ? 'Optimized' : 'Review',
          desc: inputs.ssAge >= 67 ? `Claiming at ${inputs.ssAge} maximizes lifetime benefit.` : `Claiming at ${inputs.ssAge} reduces lifetime benefit.`
        },
      ].map(risk => `
        <div class="risk-item">
  ...
  <div class="section page-break">
    <div class="section-title">30-Year Projection</div>
    <table>
      <thead>
        <tr>
          <th>Age</th>
  ...
  ${inputs.notes ? `<div class="section"><div class="section-title">Planner Notes</div><div class="notes-box">${inputs.notes}</div></div>` : ''}
  <div class="footer">
...
const DEFAULT_INPUTS: ReportInputs = {
  name: '', preparedFor: '', date: new Date().toISOString().split('T')[0],
  currentAge: 52, retireAge: 55, lifeExpectancy: 90,
  portfolio: 1_150_000, taxable: 350_000, rothIRA: 150_000, trad401k: 650_000,
  annualSpending: 55_000, healthcareCost: 8_400, inflationRate: 2.5, returnRate: 6.5,
  ssAge: 67, ssIncome: 24_000, partTimeIncome: 0, partTimeYears: 0, notes: '',
}

export default function PDFReportPage() {
  const [inputs, setInputs]       = useState<ReportInputs>(DEFAULT_INPUTS)
  ...
  useEffect(() => {
    if (!active) return
    const totalPortfolio = (active.taxable ?? 0) + (active.k401 ?? 0) + (active.roth ?? 0)
    setInputs(prev => ({
      ...prev,
      currentAge:    active.current_age     ?? prev.currentAge,
      retireAge:     active.retire_age      ?? prev.retireAge,
      lifeExpectancy: active.life_expectancy ?? prev.lifeExpectancy,
      portfolio:     totalPortfolio > 0 ? totalPortfolio : prev.portfolio,
      taxable:       active.taxable         ?? prev.taxable,
      rothIRA:       active.roth            ?? prev.rothIRA,
      trad401k:      active.k401            ?? prev.trad401k,
      annualSpending: active.spending       ?? prev.annualSpending,
      inflationRate: (active.inflation      ?? 0.025) * 100,
      returnRate:    (active.return_rate    ?? 0.065) * 100,
      ssAge:         active.ss_age          ?? prev.ssAge,
      ssIncome:      active.ss_benefit      ?? prev.ssIncome,
      partTimeIncome: active.other_income   ?? prev.partTimeIncome,
    }))
    ...
    <div className="flex items-center justify-between py-2 border-b border-white/[0.04]">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <input type={type} min={min} max={max} step={step}
          value={(inputs as any)[field]} onChange={set(field)}
      ...
      <div className="bg-[#0D1420] border-b border-white/[0.06] px-5 py-10">
        <div className="max-w-6xl mx-auto flex items-end justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-white/45 text-[14px]">One-click export. Branded, shareable, CPA-ready.</p>
          </div>
          {loadedFrom && (
            <div className="font-mono text-[9px] tracking-widest uppercase bg-[#E8B84B]/10 border border-[#E8B84B]/20 rounded-lg px-3 py-2 text-[#E8B84B]/70">
              Loaded from: <span className="text-[#E8B84B]">{loadedFrom}</span>
            </div>
          )}
        </div>
      </div>
            ...
            <div className="bg-[#141C28] border border-white/[0.07] rounded-xl overflow-hidden">
              ...
              <div className="p-5 space-y-1">
                <div className="flex items-center justify-between py-2 border-b border-white/[0.04]">
                  ...
                  <input type="text" placeholder="Your name" value={inputs.preparedFor} onChange={set('preparedFor')}
                ...
                <textarea rows={3} placeholder="Add any notes, goals, or context for your advisor..."
                  value={inputs.notes} onChange={set('notes')}
            ...
            <div className="bg-[#141C28] border border-[#E8B84B]/20 rounded-xl overflow-hidden sticky top-5">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  {['Key metrics summary','Personal & portfolio details','Spending plan analysis','4 automated risk checks','30-year projection table','Planner notes section','Disclaimer for CPA sharing'].map(item => (
                    <div key={item} className="flex items-center gap-2 text-[11px] text-white/40">
                      <span className="text-[#4ADE80] text-[9px]">✓</span>{item}
                    </div>
                  ))}
                </div>
                ...
                >
                  {generating ? <><span className="animate-spin">⟳</span> Generating...</> : <>📄 Generate PDF Report</>}
                </button>
